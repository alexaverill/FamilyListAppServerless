...
export const handler: Handler = async (event, context) => {
  ...
  let activeEvents = events.filter((event) => event.active === "true");
  if (event.pathParameters.proxy) {
    //filter by userId
    var userId = event.pathParameters.proxy;
    let filteredEvents: unknown = activeEvents.filter((event) => {
      let inGiving =
        event.giving?.findIndex((element: any) => element.userId === userId) >=
        0;
      let inRecieving =
        event.recieving?.findIndex(
